...
class DBconnection:
    def __init__(self, database_name):
        self.connection = pyodbc.connect(f"Driver=SQLite3 ODBC Driver;Database=test.db;")
        self.cursor = self.connection.cursor()
        self.cursor.execute('DROP TABLE IF EXISTS advertisement')
        self.cursor.execute('''
...
class News:
    ...
    def write_to_db(self, db_connection):
        # print(
        #     f"Inserting News into DB: type_adv='news', adv_text='{self.text}', city='{self.city}', publ_date={self.date}")
        try:
            db_connection.cursor.execute(
                '''INSERT INTO advertisement (type_adv, adv_text, city, publ_date) 
                   VALUES (?, ?, ?, ?)''',
                ('news', self.text, self.city, self.date)
            )
            db_connection.connection.commit()
            print("News successfully committed to the database")
        except Exception as e:
            print(f"Error committing News to the database: {str(e)}")
...
class Adv:
    ...
    def write_to_db(self, db_connection):
        #print(f"Inserting Adv into DB: type_adv='adv', adv_text='{self.text}', expire_date='{self.expiration_date}', days_left={self.days_left()}")
        try:
            db_connection.cursor.execute(
                '''INSERT INTO advertisement (type_adv, adv_text, expire_date, days_left) 
                   VALUES (?, ?, ?, ?)''',
                ('adv', self.text, self.expiration_date, self.days_left())
            )
            db_connection.connection.commit()
            print("Ad successfully committed to the database")
        except Exception as e:
            print(f"Error committing Ad to the database: {str(e)}")
...
class Unique:
    ...
    def write_to_db(self, db_connection):
        weather, _, _ = self.get_weather()
        #print(f"Inserting Unique into DB: type_adv='unique', adv_text='{self.text}', city='{self.city}', weather='{weather}'")
        try:
            db_connection.cursor.execute(
                '''INSERT INTO advertisement (type_adv, adv_text, city, weather) 
                   VALUES (?, ?, ?, ?)''',
                ('unique', self.text, self.city, weather)
            )
            db_connection.connection.commit()
            print("Custom record successfully committed to the database")
        except Exception as e:
            print(f"Error committing Custom record to the database: {str(e)}")
...
class JSONProcessor:
    ...
    def process_json_record(self, record, db_connection):
        try:
            ...
            if record_type == "news":
                ...
                news_item.write_to_db(db_connection)
                ...
                try:
                    ...
                    ad_item.write_to_db(db_connection)
                except ValueError:
                ...
                unique_item.write_to_db(db_connection)
            else:
                print(f"Unknown record type: {record_type}")
        except KeyError as e:
            print(f"Invalid record format, missing key: {str(e)}")

    def process_json_file(self, db_connection):
        try:
            with open(self.json_file_path, "r") as file:
                records = json.load(file)

                if isinstance(records, dict):
                    self.process_json_record(records, db_connection)
                elif isinstance(records, list):
                    for record in records:
                        self.process_json_record(record, db_connection)
                else:
...
class xmlProcessor:
    ...
    def process_xml_file(self, db_connection):
        try:
            tree = ET.parse(self.xml_file_path)
            root = tree.getroot()
            for record in root.findall('item'):
                self.process_xml_record(record, db_connection)
        except FileNotFoundError:
    ...
    def process_xml_record(self, record, db_connection):
        try:
...
class FileProcessor:
    ...
    def process_record(self, record, db_connection):
        if len(record) == 1:
    ...
    def process_file(self, db_connection):
        try:
            with open(self.file_path, "r") as file:
                ...
                for line in file:
                    ...
                    lines.append(line)
                    if len(lines) == 3:
                        self.process_record(lines, db_connection)
                        lines = []
...
def main():
    ...
    if input_type == "txt":
        ...
        processor.process_file(db_connection=db_connection)

    elif input_type == "json":
        json_file_path = input("Enter JSON file path or press Enter to use default (input_file.json): ").strip() or "input_file.json"
        json_processor = JSONProcessor(json_file_path=json_file_path)
        json_processor.process_json_file(db_connection=db_connection)
        ...
        xml_processor.process_xml_file(db_connection=db_connection)
    else:
